...
export interface PreparedRead {
  ...
  /** Input after the operation's own schema parsed it. */
  input: unknown;
}

/** Checks a source against the registry without running anything. */
export function prepareRead(registry: ServerModuleRegistry | null, raw: unknown): PreparedRead {
  ...
  if (!input.success) {
    const issues = input.error.issues.map((i) => ({ path: i.path.join('.') || '(root)', message: i.message }));
    throw new AppError(
      'validation_failed',
  ...
  return { source, entry, input: input.data };
}
